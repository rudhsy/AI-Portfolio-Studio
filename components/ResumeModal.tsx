...
import { X, Download, ExternalLink, Mail, MapPin, Calendar } from 'lucide-react';
...
const ResumeModal: React.FC<ResumeModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div 
      ref={overlayRef}
      className="fixed inset-0 z-[60] bg-gray-200/90 dark:bg-black/90 backdrop-blur-sm hidden items-center justify-center overflow-y-auto p-4 md:p-8"
    >
      {/* Close Button */}
      <button 
        onClick={onClose}
        className="fixed top-6 right-6 p-3 rounded-full bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all z-50 text-black dark:text-white shadow-lg hover:rotate-90"
      >
        <X size={24} />
      </button>

      <div ref={contentRef} className="w-full max-w-5xl my-auto">
        
        {/* Paper Container */}
        <div className="bg-white dark:bg-[#111] shadow-2xl border border-gray-200 dark:border-gray-800 rounded-sm overflow-hidden relative">
